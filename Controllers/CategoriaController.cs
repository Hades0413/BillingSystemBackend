...
public class CategoriaController : ControllerBase
{
    ...
    public async Task<IActionResult> Registrar([FromBody] Categoria categoria)
    {
        ...
        try
        {
            var (success, mensaje, categoriaRegistrada) =
                await _categoriaService.RegistrarCategoriaAsync(categoria.UsuarioId, categoria.CategoriaNombre);
            if (!success) return BadRequest(new ErrorResponse(mensaje));

            return CreatedAtAction(nameof(ListarCategoriasConUsuarioId), new { usuarioId = categoria.UsuarioId },
                new SuccessResponse(mensaje, categoriaRegistrada));
        }
        catch (Exception ex)
        {
    ...
    {
        if (categoriaId <= 0) return BadRequest(new ErrorResponse("ID de categoría inválido."));
        ...
        try
        {
            ...
            if (!result.success) return BadRequest(new ErrorResponse(result.message));
