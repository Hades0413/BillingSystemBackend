...
public class EmpresaController : ControllerBase
{
    ...
    {
        if (empresa == null) return BadRequest(new ErrorResponse("La empresa no puede ser nula."));
        ...
        try
        {
            ...
            if (empresaId > 0) return Ok(new SuccessResponse("Empresa registrada exitosamente.", empresa));
        ...
        catch (Exception ex)
        {
            return StatusCode(500,
                new ErrorResponse("Error interno del servidor al registrar la empresa.", ex.Message));
        }
    }
    ...
    {
        if (usuarioId <= 0) return BadRequest(new ErrorResponse("El ID del usuario debe ser mayor a cero."));
    ...
    {
        if (string.IsNullOrWhiteSpace(ruc)) return BadRequest(new ErrorResponse("El RUC es obligatorio."));
